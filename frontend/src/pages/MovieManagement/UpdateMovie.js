...
const UpdateMovie = () => {
  ...
  const getSingleMovie = async () => {
    try {
      const { data } = await axios.get(
        `http://localhost:8080/api/v1/movies/get-movie/${params.slug}`
      );
  ...
  const getAllMovieCategory = async () => {
    try {
      const { data } = await axios.get(
        "http://localhost:8080/api/v1/moviecategory/get-moviecategory"
      );
      if (data?.success) {
  ...
  const handleDelete = async () => {
    try {
      ...
      const { data } = await axios.delete(
        `http://localhost:8080/api/v1/movies/delete-movie/${id}`
      );
