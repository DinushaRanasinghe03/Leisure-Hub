...
//dulanka
// get movie schedules
export const getMovieSchedulesController = async (req, res) => {
  try {
    ...
    const movieSchedules = await movieScheduleModel.find({ movie: movieId });

    res.status(200).send({
      success: true,
      message: "A movie with schedules",
      movieSchedules,
    });
  } catch (error) {
    console.log(error);
...
//Tiny
//get movie title from the movies collection
export const getMovieTitleController = async (req, res) => {
  try {
    const { id } = req.params; // Corrected to destructure id directly

    // Find the movie in the database using the ID
    const movie = await movieModel.findOne({ _id: id }).select("name");
    ...
    // If the movie is found, send its name in the response
    res.status(200).json({
      success: true,
